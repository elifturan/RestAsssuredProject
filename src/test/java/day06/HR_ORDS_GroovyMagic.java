...
//All IDs [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122, 123, 124]
        // print first id and last id
        System.out.println("first id " + jp.getInt("items[0].employee_id")); //first id 100
        System.out.println("last  id " + jp.getInt("items[-1].employee_id")); //last  id 124
        ...
        //from the first till the fifth [100, 101, 102, 103, 104]
        // get all last names from index 10-15
        System.out.println("last name index from 10-15"  + jp.getList("items[10..15].last_name"));
//last name index from 10-15[Chen, Sciarra, Urman, Popp, Raphaely, Khoo]
        System.out.println("first names "+ jp.getList(("items[3..6].first_name")));
//first names [Alexander, Bruce, David, Valli]
        ...
        System.out.println("result = " + result); //result = Austin
        // using above example : find the salary of employee with email value LDEHAAN

        int employeeId= jp.getInt("items.find{it.email=='LDEHAAN'}.employee_id");
        System.out.println(employeeId); //102

        int salary =  jp.getInt("items.find { it.email=='LDEHAAN'}.salary ") ;
        System.out.println("salary = " + salary); //salary = 17000
        ...
        String firstGuy=jp.getString("items.max{it.salary}.salary");
    }
